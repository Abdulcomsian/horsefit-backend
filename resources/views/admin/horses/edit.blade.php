...
<div class="card">
    <div class="card-header">
        {{ trans('global.edit') }} {{ trans('cruds.horse.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.horses.update", [$horse->id]) }}" enctype="multipart/form-data">
            @method('PUT')
            @csrf
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('name') ? 'is-invalid' : '' }}" type="text" name="name" id="name" value="{{ old('name', $horse->name) }}" required>
                @if($errors->has('name'))
                    <div class="invalid-feedback">
                        {{ $errors->first('name') }}
                    </div>
            ...
            <div class="form-group">
                ...
                <select class="form-control {{ $errors->has('type') ? 'is-invalid' : '' }}" name="type" id="type" required>
                    ...
                    @foreach(App\Models\Horse::TYPE_SELECT as $key => $label)
                        <option value="{{ $key }}" {{ old('type', $horse->type) === (string) $key ? 'selected' : '' }}>{{ $label }}</option>
                    @endforeach
                </select>
                @if($errors->has('type'))
                    <div class="invalid-feedback">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('nationality') ? 'is-invalid' : '' }}" type="text" name="nationality" id="nationality" value="{{ old('nationality', $horse->nationality) }}" required>
            ...
            <div class="form-group">
                ...
                <input class="form-control date {{ $errors->has('date_of_birth') ? 'is-invalid' : '' }}" type="text" name="date_of_birth" id="date_of_birth" value="{{ old('date_of_birth', $horse->date_of_birth) }}" required>
            ...
            <div class="form-group">
                ...
                <select class="form-control {{ $errors->has('gender') ? 'is-invalid' : '' }}" name="gender" id="gender" required>
                    ...
                    @foreach(App\Models\Horse::GENDER_SELECT as $key => $label)
                        <option value="{{ $key }}" {{ old('gender', $horse->gender) === (string) $key ? 'selected' : '' }}>{{ $label }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <select class="form-control {{ $errors->has('blood_type') ? 'is-invalid' : '' }}" name="blood_type" id="blood_type" required>
                    ...
                    @foreach(App\Models\Horse::BLOOD_TYPE_SELECT as $key => $label)
                        <option value="{{ $key }}" {{ old('blood_type', $horse->blood_type) === (string) $key ? 'selected' : '' }}>{{ $label }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('mother_name') ? 'is-invalid' : '' }}" type="text" name="mother_name" id="mother_name" value="{{ old('mother_name', $horse->mother_name) }}">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('father_name') ? 'is-invalid' : '' }}" type="text" name="father_name" id="father_name" value="{{ old('father_name', $horse->father_name) }}">
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('trainers') ? 'is-invalid' : '' }}" name="trainers[]" id="trainers" multiple>
                    @foreach($trainers as $id => $trainer)
                        <option value="{{ $id }}" {{ (in_array($id, old('trainers', [])) || $horse->trainers->contains($id)) ? 'selected' : '' }}>{{ $trainer }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('owners') ? 'is-invalid' : '' }}" name="owners[]" id="owners" multiple>
                    @foreach($owners as $id => $owner)
                        <option value="{{ $id }}" {{ (in_array($id, old('owners', [])) || $horse->owners->contains($id)) ? 'selected' : '' }}>{{ $owner }}</option>
                    @endforeach
                </select>
