    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.posts.update", [$post->id]) }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <textarea class="form-control {{ $errors->has('body') ? 'is-invalid' : '' }}" name="body" id="body" required>{{ old('body', $post->body) }}</textarea>
                @if($errors->has('body'))
            ...
            <div class="form-group">
                <div class="form-check {{ $errors->has('status') ? 'is-invalid' : '' }}">
                    ...
                    <input class="form-check-input" type="checkbox" name="status" id="status" value="1" {{ $post->status || old('status', 0) === 1 ? 'checked' : '' }}>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('visibility') ? 'is-invalid' : '' }}" name="visibility" id="visibility" required>
                    @foreach($visibilityOptions as $id => $entry)
                        <option value="{{ $id }}" {{ old('visibility', $post->visibility) == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
...
Dropzone.options.mediaDropzone = {
    ...
    removedfile: function (file) {
      ...
      var name = ''
      if (typeof file.media_link !== 'undefined') {
        name = file.media_link
      } else {
    ...
    @if(isset($post) && $post->media)
          var files = {!! json_encode($post->media) !!}
              ...
              this.options.thumbnail.call(this, file, file.media_link ?? '')
              file.previewElement.classList.add('dz-complete')
              $('form').append('<input type="hidden" name="media[]" value="' + file.media_link + '">')
            }
    @endif
    },
