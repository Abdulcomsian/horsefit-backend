<template>
  <AuthenticatedLayout title="Ppdate Module">
      <template #header>
        <h2 class="font-semibold text-xl text-gray-800 leading-tight">Add New Module</h2>
      </template>

    <Container>
        <FormSectionCustom @submitted="updateModule" class="">
          <template #form>
              ...
              <CloneComponent
                ...
                :itemArray="props.module.permissions"
...
const cloneInputData = (value) => {
    ...
    if (value[initialInputData.value.length - 1].id === undefined) {
...
const updateModule = async () => {
    axios.put(route('modules.update', props.module.id), form)
    .then(response => {
