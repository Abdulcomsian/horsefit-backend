<template>
  <AuthenticatedLayout title="Modules">
    ...
    <template #header>
      ...
      <Container>
        <Filters
            ...
            print-title="Print Module Table"
            print-id="printModuleTable"
            :btnCreate="route('modules.create')"
            :export-data="modules.data"
            :search-query="requestParam === null ? '' : requestParam.search"
            :search-entries="requestParam === null ? 10 : requestParam.entries"
            :filterData="filterData"
            :file-name="'Modules'"
            :has-create="$page.props.permissions.includes('Create Module')"
            :is-enable-export="$page.props.permissions.includes('Export Module')"
            :is-enable-print="$page.props.permissions.includes('Print Module')"
        />
        <div class="relative overflow-x-auto shadow-md sm:rounded-lg mt-3" id="printModuleTable">
          <table class="w-full text-sm text-left rtl:text-right text-gray-500 dark:text-gray-400">
            ...
            <tr v-else v-for="module in modules.data" :key="module.id"
                class="odd:bg-white odd:dark:bg-gray-900 even:bg-gray-50 even:dark:bg-gray-800 border-b dark:border-gray-700">
              <td class="px-6 py-4 dark:text-white">{{ module.name }}</td>
              <td class="px-6 py-4 dark:text-white">
                {{ dayjs(module.created_at).format('dddd, MMMM d, YYYY') }}
              </td>
              <td class="px-6 py-4 dark:text-white text-start">
                <Status :status="changeStatus(module.status)"/>
              </td>
              <td class="dark:text-white hide-on-print">
                <Link v-if="$page.props.permissions.includes('Edit Module')" :href="route('modules.edit', {'id': module.id})"
                ...
                <button
                    v-if="$page.props.permissions.includes('Delete Module')"
                    type="button"
                    ...
                    @click="deleteConfirmation(module)"
                >
        ...
        <Pagination :data-object="modules"/>
      </Container>
...
const selectedViewModule = ref(null);

const showModuleDetails = (item) => {
  selectedViewModule.value = item;
...
const deleteModule = (item) => {
  ...
  try {
    ...
    deleteForm.delete(route('modules.destroy', {id: item.id}), {
      preserveState: true,
      preserveScroll: true,
      onSuccess: () => {
...
const filterData = () => {
  ...
  filteredTypes.value = props.modules.data.map((module, index) => {
    return {
      No: index + 1,
      Name: module.name,
      'Created At':  module.created_at ? dayjs(module.created_at).format('dddd, MMMM d, YYYY')  : ''
    };
  });
